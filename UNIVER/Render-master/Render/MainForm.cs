...
namespace Render
{
    public partial class MainForm : Form
    {
        ...
        private void Numeric_ValueChanged(object sender, EventArgs e)
        {
            if (MainUnitProcessor.Model != null)
            {
                MyDrawing.alphaAngle = Math.PI - (double)_ctrlNumAngleAlf.Value;
        ...
        private void _ctrlButTriangle_Click(object sender, EventArgs e)
        {
            MainUnitProcessor.ModelDivide((int)_ctrlNumPolyg.Value);
            MainUnitProcessor.Model.SetTurning(0);
            DrawImage();
        }

        private void _ctrlButCent_Click(object sender, EventArgs e)
        {
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
                    MainUnitProcessor.ParseTexture(openFileDialog1.FileName);
        }
    }
}
