...
namespace Draw3D
{
    public partial class FormGraph : Form
    {
        ...
        private void поискВГлубинуToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                int n = Lib.graph.DepthSearch(0, Graph.Nodes[Lib.NumNode].name);
                //Caption:=IntToStr(n);
                MyDraw(false);
            }
        ...
        private void поискВШиринуToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (Program.formMyDialog.ShowDialog() == DialogResult.OK)
            {
                int n = Lib.graph.BreadthSearch(0, Graph.Nodes[Lib.NumNode].name);
                //Caption:=IntToStr(n);
                Text = "Path: " + Lib.myStack.StackToStr();
                MyDraw(false);
            }
        ...
        {
            MessageBox.Show(""+Graph.FindWin2(textBox1.Text));
                MyDraw(false);
        }
        ...
        {
            Lib.graph.SetFull();
            Program.formListBox.fl_proc = 0;//Через Гамильтон ищем все возможные пути и отправляем их в стек
            if (Program.formListBox.ShowDialog() == DialogResult.OK) { };
            MessageBox.Show(Graph.Go());
            MyDraw(false);
        }

        private void MyButton3_Click(object sender, EventArgs e)
        {
            Graph.S = "Пути меньше 100 км:"+Environment.NewLine;
            Lib.graph.MinDist0(0);
            Graph.Go2(0, "0", 0);
            MessageBox.Show(Graph.S);
        }
    }
}
