...
namespace Draw3D
{
    ...
    public partial class Graph
    {
        ...
        public static TNode[] Nodes = new TNode[0];     // узлы
        public static TNode SelectNode;                 // выделенный
        ...
        public static void ChangeA(TNode node)
        {
            int a, b;
            if (node.Edge != null)
                for (int i = 0; i < node.Edge.Length; i++)
                {
                    ...
                    a = Nodes[n].x;
                    b = Nodes[n].y;
                    node.Edge[i].A = (int)Math.Sqrt((node.x - a) * (node.x - a) + (node.y - b) * (node.y - b))/10;
            ...
            for (int i = 0; i < Nodes.Length; i++)
            {
                ...
                for (int j = 0; j < Nodes.Length; j++)
                    if (i != j)
                    {
                        ...
                        e.A = (int)Math.Sqrt((a2 - a1) * (a2 - a1) + (b2 - b1) * (b2 - b1))/10;
        ...
        {
            //string s = Graph.FinalPut;
            for (int i = 0; i < s.Length - 1; i++)
            {
                int now = Int32.Parse("" + s[i]);
                int next = Int32.Parse("" + s[i + 1]);
                int last;
                if (now != 0) last = Int32.Parse("" + s[i - 1]);
                else last = Int32.Parse("" + s[s.Length - 2]);

                //Array.Resize<TEdge>(ref Nodes[now].Edge, 1);
                Nodes[now].Edge = new TEdge[2];
                TEdge e = new TEdge();

                double a1 = Nodes[now].x;
                double b1 = Nodes[now].y;
                double a2 = Nodes[next].x;
                double b2 = Nodes[next].y;

                e.A = (int)Math.Sqrt((a2 - a1) * (a2 - a1) + (b2 - b1) * (b2 - b1));
                e.x1c = Lib.graph.x1 - Nodes[now].x;
                e.x2c = Lib.graph.x2 - Nodes[next].x;
                e.yc = (Nodes[next].y + Nodes[now].y) / 2;
                e.numNode = next;
                e.color = Color.Silver;
                ...
                e.A = (int)Math.Sqrt((a2 - a1) * (a2 - a1) + (b2 - b1) * (b2 - b1));
                e.x1c = Lib.graph.x1 - Nodes[now].x;
                e.x2c = Lib.graph.x2 - Nodes[last].x;
                e.yc = (Nodes[last].y + Nodes[now].y) / 2;
                e.numNode = last;
                e.color = Color.Silver;
        ...
        public void AddEdge()  // добавить ребро
        {
            ...
            SelectNodeBeg.Edge[L - 1].A = (int)Math.Sqrt((a2 - a1) * (a2 - a1) + (b2 - b1) * (b2 - b1))/10;
