...
namespace ООП3__Часы_с_паттернами_
{
    ...
    class Drawer : IDraw
    {
        ...
        public Drawer(Pen p, SolidBrush b, Graphics g)//конструктор для первого объявления
        {
            myPen = p;
            myBrush = b;
            graph = g;
            ...
            //куча бесполезных переменных дальше
            height = 300;
            start = 10;
            lenght = 320;
            midrad = 5;
        ...
        public Drawer(Drawer d)//для последующих
        {
            myPen = d.myPen;
            myBrush = d.myBrush;
            graph = d.graph;
        ...
        public virtual void Draw(string time, string date)//суть паттерна: при смене часов переписываем метод их отрисовки(циферблатные или цифровые)
        {

        }
    }
    class DrawerDigital : Drawer//отрисовка электронных часов
    {
        ...
        //    clockCheck = true;
        //}
        public DrawerDigital(Drawer d):base(d)
        {
            clockCheck = true;
        }
    ...
    class DrawerOldClock : Drawer//отрисовка циферблатных часов
    {
        public DrawerOldClock(Drawer d): base(d)
        {
            clockCheck = false;
        }
        public override void Draw(string time, string date)
